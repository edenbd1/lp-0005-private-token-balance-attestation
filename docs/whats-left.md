...
| # | Item | Why |
|---|---|---|
| 1 | **Vendor or symlink the SPEL wrapper for the verifier program** inside `_external/lez` (see [ADR-002](./decisions/002-verifier-program-shape.md)). | The on-chain path needs a deployable LEZ program; the kernel is ready, only the `#[lez_program]` skin is missing. |
| 2 | **Build a Logos Delivery bridge** (Qt subprocess or `liblogosdelivery` FFI — see task #16). | Required for the off-chain path's real-network demo. |
| 3 | **Deploy the verifier program on LEZ testnet** and record the program ID. | Submission requirement. |
| 4 | **Wire the sequencer client** (see [`sequencer-client-plan.md`](./sequencer-client-plan.md)). | Currently the SDK uses synthesized Merkle paths; real flow needs `getProofForCommitment`. |
...
| 6 | **Re-add CI workflow** (minimal: `cargo build --workspace`). | Submission requirement: "CI must be green on the default branch." |
| 7 | **Record the demo video** narrating the architecture and walking through the on-chain + off-chain paths with `RISC0_DEV_MODE=0` visible. | Submission requirement. |
